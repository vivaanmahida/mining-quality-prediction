...
def generate_and_convert():
    ...
    # Code and Report Submission EXACT links per upskillcampus instructions
    code_url   = "https://github.com/vivaanmahida/upskillcampus/blob/main/QualityPredictionInAMiningProcess.py"
    report_url = "https://github.com/vivaanmahida/upskillcampus/blob/main/QualityPredictionInAMiningProcess_Vivaan_USC_UCT.pdf"

    for p in doc.paragraphs:
        if "Code submission (Github link)" in p.text:
            p.text = f"Code submission (Github link):\n{code_url}"
        if "Report submission (Github link)" in p.text:
            p.text = f"Report submission (Github link):\n{report_url}"
    ...
    if p_des_idx != -1:
        doc.paragraphs[p_des_idx].text = (
            ...
            "1. Data Preprocessor (QualityPredictionInAMiningProcess.py): Cleans sensor data, handles timestamps, and resamples 20s data into 1-minute and 1-hour uniform grids.\n"
            "2. Feature Engineer: Computes autoregressive lag features (target_lag_1 to target_lag_60), rolling window statistics (5m to 60m mean/std), cyclical time encodings, and multi-step target horizons (1h to 12h ahead).\n"
            "3. Model Trainer: Executes chronological train/test splits (80% train, 20% test) and trains XGBoost, LightGBM, and Random Forest regressors across 3 experiments.\n"
            "4. Dashboard Generator: Encapsulates model outputs, feature importances, and predictions into a standalone Plotly HTML dashboard (dashboard/index.html)."
        )
    ...
    # Save output docx with exact requested naming
    docx_name = 'QualityPredictionInAMiningProcess_Vivaan_USC_UCT.docx'
    out_docx1 = os.path.join(r'D:\Projects\Mining Quality Prediction', docx_name)
    out_docx2 = os.path.join(r'D:\Projects\Mining Quality Prediction\mining-quality-prediction', docx_name)
    
    doc.save(out_docx1)
    doc.save(out_docx2)
    print("Saved DOCX:", out_docx2)

    # Convert to PDF using MS Word
    pdf_name = 'QualityPredictionInAMiningProcess_Vivaan_USC_UCT.pdf'
    out_pdf1 = os.path.join(r'D:\Projects\Mining Quality Prediction', pdf_name)
    out_pdf2 = os.path.join(r'D:\Projects\Mining Quality Prediction\mining-quality-prediction', pdf_name)

    word = win32com.client.Dispatch('Word.Application')
    word.Visible = False
    
    doc_word = word.Documents.Open(os.path.abspath(out_docx2))
    doc_word.SaveAs(os.path.abspath(out_pdf2), FileFormat=17) # 17 = wdFormatPDF
    doc_word.Close()
    word.Quit()
    
    # Copy to root as well
    with open(out_pdf2, 'rb') as f_in, open(out_pdf1, 'wb') as f_out:
